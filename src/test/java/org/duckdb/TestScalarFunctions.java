            ...
            DuckDBFunctions.scalarFunction()
                ...
                .withVectorizedFunction(ctx -> {
                    ...
                    ctx.stream().forEachOrdered(row -> {
                        try {
                            assertThrows(() -> { row.getBoolean(0); }, IllegalStateException.class);
                            assertTrue(row.getBoolean(0, true));
                            assertThrows(() -> { row.getByte(1); }, IllegalStateException.class);
                            assertEquals(row.getByte(1, (byte) 42), (byte) 42);
                            assertThrows(() -> { row.getUint8(2); }, IllegalStateException.class);
                            assertEquals(row.getUint8(1, (short) 42), (short) 42);
                            assertThrows(() -> { row.getShort(3); }, IllegalStateException.class);
                            assertEquals(row.getShort(1, (short) 42), (short) 42);
                            assertThrows(() -> { row.getUint16(4); }, IllegalStateException.class);
                            assertEquals(row.getUint16(1, 42), 42);
                            assertThrows(() -> { row.getInt(5); }, IllegalStateException.class);
                            assertEquals(row.getInt(1, 42), 42);
                            assertThrows(() -> { row.getUint32(6); }, IllegalStateException.class);
                            assertEquals(row.getUint32(1, (long) 42), (long) 42);
                            assertThrows(() -> { row.getLong(7); }, IllegalStateException.class);
                            assertEquals(row.getLong(1, (long) 42), (long) 42);
                            assertThrows(() -> { row.getFloat(8); }, IllegalStateException.class);
                            assertEquals(row.getFloat(1, (float) 42.1), (float) 42.1);
                            assertThrows(() -> { row.getDouble(9); }, IllegalStateException.class);
                            assertEquals(row.getDouble(1, 42.1), 42.1);
                        } catch (Exception e) {
